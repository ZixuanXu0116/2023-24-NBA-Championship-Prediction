...
for season in tqdm(range(2016, 2025), desc="Processing years"):
    ...
    for game in game_data_list:
        ...
        if len(team1_players) >= 9 and len(team2_players) >= 9:
            ...
            [(player_data['Tm'] == team1)].drop(columns = ['Player', 'Year', 'Tm', 'G', 'season']).reset_index(drop=True)
            team2_matrix = player_data[player_data['Player'].isin(team2_players)]\
            [(player_data['Tm'] == team2)].drop(columns = ['Player', 'Year', 'Tm', 'G', 'season']).reset_index(drop=True)
