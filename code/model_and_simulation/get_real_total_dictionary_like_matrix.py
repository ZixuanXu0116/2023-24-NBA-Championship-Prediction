...
def load_player_cluster_data(season):
    query = (
        f'SELECT * FROM players_ability_cluster_regular_data WHERE "Year" = {season}'
    )
...
for season in tqdm(range(2015, 2025), desc='Processing years'):
    ...
    game_start_indices = game_by_game_data.index[
        game_by_game_data['Home'].ne(game_by_game_data['Home'].shift())
    ].tolist()
    ...
    for game in game_data_list:
        ...
        selected_game = game_by_game_data[
            (game_by_game_data['Tm'] == team1) | (game_by_game_data['Tm'] == team2)
        ]
        ...
        if len(team1_players) >= 9 and len(team2_players) >= 9:
            team1_features = team_data[team_data['Team'] == team1][
                ['ORtg', 'DRtg', 'NRtg', 'TOV%', 'ORB%', 'eFG%']
            ]
            team1_features.columns = [
                'ORtg_1',
                'DRtg_1',
                'NRtg_1',
                'TOV%_1',
            ...
            team2_features = team_data[team_data['Team'] == team2][
                ['ORtg', 'DRtg', 'NRtg', 'TOV%', 'ORB%', 'eFG%']
            ]
            team2_features.columns = [
                'ORtg_2',
                'DRtg_2',
                'NRtg_2',
                'TOV%_2',
            ...
            team1_matrix = (
                player_data[player_data['Player'].isin(team1_players)][
                    (player_data['Tm'] == team1)
                ]
                .drop(columns=['Player', 'Year', 'Tm', 'G'])
                .reset_index(drop=True)
            )
            team2_matrix = (
                player_data[player_data['Player'].isin(team2_players)][
                    (player_data['Tm'] == team2)
                ]
                .drop(columns=['Player', 'Year', 'Tm', 'G'])
                .reset_index(drop=True)
            )
