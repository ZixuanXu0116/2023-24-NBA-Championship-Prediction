...
def get_df_stats(season, game_type, target_type, data_type):
    ...
    columns = [
        th.getText()
        for th in table[
            1 if (target_type, data_type) == ('team', 'advanced') else 0
        ].findAll('th')
    ][1:]
    ...
    if target_type == 'player':
        stats = [
            [td.getText() for td in table[1:][i].findAll('td')]
    ...
    if (game_type, target_type, data_type) == ('regular', 'player', 'normal'):
        ...
        df.columns = (
            df.columns[:-8].tolist()
            + [col + '_oppo' for col in df.columns[-8:-4]]
            + df.columns[-4:].tolist()
        )
        df_final = df
        ...
        df.columns = (
            df.columns[:18].tolist()
            + [col + '_oppo' for col in df.columns[18:-1]]
            + [df.columns[-1]]
        )
        df_final = df
            ...
            for game_type in ['regular', 'playoffs']:
                if game_type == 'regular':
                    for season in range(2015, 2025):
                        ...
                        df.to_sql(
                            f'nba_{game_type}_{data_type}_{target_type}_data',
                            con=engine,
                            if_exists='append',
                            index=False,
                        )
