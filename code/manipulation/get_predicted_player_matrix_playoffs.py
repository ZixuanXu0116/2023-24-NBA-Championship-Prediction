    ...
    for season in tqdm(range(2016, 2025), desc='Processing years'):
        ...
        for index, player_cur_playoffs in df_cur_regular.iterrows():
            ...
            if player_name in df_last_playoffs['Player'].values:
                ...
                player_attributes = df_last_playoffs[
                    df_last_playoffs['Player'] == player_name
                ].iloc[0]
            ...
            player_cluster_matrix = player_cluster_matrix.append(
                player_attributes, ignore_index=True
            )

    player_cluster_matrix.to_sql(
        'playoffs_predicted_player_matrix_data',
        con=engine,
        if_exists='replace',
        index=False,
    )
