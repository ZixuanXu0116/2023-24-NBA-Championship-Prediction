...
def get_df_stats(season):
    ...
    team_stats = [[td.getText() for td in table[i].findAll('td')] for i in range(2, 32)]
...
for season in tqdm(range(2015, 2025), desc='Processing seasons'):
    ...
    df.to_sql('nba_regular_team_adv_data_15_24', con=engine, if_exists='append', index=False)
    time.sleep(delay)